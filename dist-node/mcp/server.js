...
const usageMarkdown = () => {
    return [
        ...
        "- 图书馆电子资源/馆藏/座位校区（csu.library_*）",
        "- 校车查询（csu.bus，站点需从预设列表选择）",
        "- 校园卡信息/流水（csu.ecard_card / csu.ecard_turnover）",
        "- 多数工具会返回便于后续操作的明细 URL（如图书详情、校车班次详情、校园卡流水等），可在外部直接跳转或扩展。",
        "",
        "## 常用场景提示",
        "- 生成学习/简历 Markdown：依次调用 csu.grade（可选 term）、csu.rank、csu.level_exam、csu.student_plan、csu.minor_info，整理为分节表格，最后附总评小结。",
        "- 校园卡近期开销：调用 csu.ecard_turnover（给定时间范围），把每条记录以表格列出时间、商户、金额、备注、余额，并在末尾汇总支出/收入统计。",
        "- 校车班次 Markdown：调用 csu.bus 后按 StartTime 排序，输出表格（包含 id、时间、途径站），并在每行附 DetailUrl 便于跳转查看原页面。",
        "- 课表渲染：调用 csu.classes，按 TimeInWeek（周日=1）和节次生成 7 列周视图或分日列表，处理同一时间段的多课程并标注周次范围。",
        "- 图书馆座位参考：调用 csu.library_seat_campuses，返回的 SeatUrl 可直接用于预约（人工在浏览器打开）。",
        "- 图书查询：调用 csu.library_book_search，表格输出书名/作者/馆藏状态，并在每条记录附带 bookDetails 链接（url 字段）供用户点击跳转。",
        "",
        "## 使用提示",
...
export const createMcpServer = (opts) => {
    ...
    toolDefs.push({
        meta: {
            name: "csu.grade",
            description: "查询成绩列表，需学号/密码，可选 term（示例：2024-2025-1 / 2023-2024-2）,否则为全部学期数据。",
            inputSchema: {
                type: "object",
                properties: {
                    ...credentialProps,
                    term: {
                        type: "string",
                        description: "学年学期,（格式如 2025-2026-1、2024-2025-2 等）,可留空(代表全部学期)",
                    },
                },
                required: ["id", "pwd"],
            },
    ...
    toolDefs.push({
        meta: {
            ...
            description: "校园卡流水查询，timeFrom/timeTo/amountFrom/amountTo 可选",
            inputSchema: {
                type: "object",
                properties: {
                    ...credentialProps,
