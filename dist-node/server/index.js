...
import { classes, grade, rank, levelExam, studentInfo, minorInfo, studentPlan, summaryMarkdown, } from "./jwc.js";
import { searchLibraryDb, searchLibraryBooks, fetchBookCopies, fetchSeatCampuses, } from "./library.js";
...
app.get("/api/jwc/:id/:pwd/grade", async (req, res) => {
    try {
        const grades = await grade({ id: req.params.id, pwd: req.params.pwd }, typeof req.query.term === "string" ? req.query.term : "");
...
app.get("/api/jwc/:id/:pwd/class/:term/:week", async (req, res) => {
    try {
        const { classes: cls, startWeekDay } = await classes({ id: req.params.id, pwd: req.params.pwd }, req.params.term, req.params.week);
        res.json({
            StateCode: 1,
            Error: "",
            Class: cls,
...
app.get("/api/jwc/:id/:pwd/summary", async (req, res) => {
    try {
        const term = "";
        const md = await summaryMarkdown({ id: req.params.id, pwd: req.params.pwd }, term);
...
app.get("/api/library/:id/:pwd/booksearch", async (req, res) => {
    try {
        ...
        const data = await searchLibraryBooks({ id: req.params.id, pwd: req.params.pwd }, kw);
        res.json({
            StateCode: 1,
            Error: "",
            Status: data.status,
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error("[server][library booksearch] error:", message, error);
        res.json({
            StateCode: -1,
            Error: message,
            Status: 0,
...
app.get("/api/library/:id/:pwd/bookcopies/:recordId", async (req, res) => {
    try {
        ...
        const data = await fetchBookCopies({ id: req.params.id, pwd: req.params.pwd }, recordId);
        res.json({
            StateCode: 1,
            Error: "",
            Status: data.status,
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error("[server][library bookcopies] error:", message, error);
        res.json({
            StateCode: -1,
            Error: message,
            Status: 0,
...
app.get("/api/library/seat/campuses", async (_req, res) => {
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error("[server][library seat campuses] error:", message, error);
        res.json({
            StateCode: -1,
            Error: message,
            Status: 0,
