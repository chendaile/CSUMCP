...
import { createSessionFetch, encryptPassword, } from "./auth.js";
...
const opacCasLoginURL = `https://ca.csu.edu.cn/authserver/login?service=${encodeURIComponent(OPAC_SERVICE_URL)}`;
const LIB_DB_SEARCH_URL = "https://libdb.csu.edu.cn/accessData";
const OPAC_UNIFY_SEARCH_URL = "https://opac.lib.csu.edu.cn/find/unify/search";
const OPAC_GROUP_ITEMS_URL = "https://opac.lib.csu.edu.cn/find/physical/groupitems";
...
export const loginLibrary = async (username, password) => {
    ...
    debug("loginLibrary page status", loginPage.status, "url", loginPage.url);
    ...
    debug("loginLibrary response status", resp.status, "redirect", resp.url);
    ...
    if (finalCookies) {
        debug("loginLibrary cookies:", JSON.stringify(finalCookies.cookies.map((c) => ({
            key: c.key,
            domain: c.domain,
            path: c.path,
        }))));
...
export const loginLibraryOpac = async (username, password) => {
    debug("loginLibraryOpac start", { username, pwdLen: password.length });
    const { sessionFetch, jar } = createSessionFetch();
    const loginPage = await sessionFetch(opacCasLoginURL);
    debug("loginLibraryOpac page status", loginPage.status, "url", loginPage.url);
    const html = await loginPage.text();
    ...
    const unifyIndexResp = await sessionFetch("https://opac.lib.csu.edu.cn/find/unify/index", { redirect: "follow" });
    ...
    const cookies = Array.isArray(finalCookies.cookies)
        ...
        : [];
    const hasOpacCookie = cookies.some((c) => String(c.domain ?? "").includes("opac.lib.csu.edu.cn"));
    debug("loginLibraryOpac cookies:", JSON.stringify(cookies.map((c) => ({
        key: c.key,
        domain: c.domain,
        path: c.path,
    }))));
...
export const authenticatedLibraryRequest = async (username, password, method, url, body) => {
...
export const searchLibraryBooks = async (_user, kw) => {
    ...
    const searchUrl = `https://opac.lib.csu.edu.cn/#/searchList?keyWord=${encodeURIComponent(kw)}`;
...
export const fetchBookCopies = async (_user, recordId) => {
    ...
    const detailsUrl = recordId && String(recordId)
        ? `https://opac.lib.csu.edu.cn/#/searchList/bookDetails/${recordId}`
        : "";
    let parsed;
    try {
...
export const fetchSeatCampuses = async () => {
    ...
    try {
        ...
        parsed.Campuses = await Promise.all(parsed.Campuses.map(async (campus) => {
            try {
                ...
                const childArea = Array.isArray(list?.childArea)
                    ? list.childArea
                    : [];
                const floors = childArea.map((f) => ({
                    Id: Number(f.id ?? 0),
                    Name: f.name ?? "",
                    Total: Number(f.TotalCount ??
                        0),
                    Unavailable: Number(f.UnavailableSpace ??
                        0),
                    Remaining: Math.max(0, Number(f.TotalCount ??
                        0) -
...
const parseDbList = (list, base) => {
    ...
    list.find(".lib-data-body .row").each((_, row) => {
        ...
        const accessLink = $row("a[href^='javascript:jinru']").attr("href");
        let accessId = "";
        if (accessLink) {
...
export const searchLibraryDb = async (elecName) => {
    ...
    debug("searchLibraryDb parsed", "cn", chinese.length, "foreign", foreign.length);
