...
const ECARD_SERVICE_URL = "https://ecard.csu.edu.cn/berserker-auth/cas/login/wisedu?targetUrl=https://ecard.csu.edu.cn/plat-pc/?name=loginTransit";
const ECARD_CAS_LOGIN_URL = `https://ca.csu.edu.cn/authserver/login?service=${encodeURIComponent(ECARD_SERVICE_URL)}`;
const debug = (...args) => {
    logger.info("[ecard]", ...args);
};
export const loginEcard = async (username, password) => {
    ...
    if (finalCookies) {
        debug("loginEcard cookies:", JSON.stringify(finalCookies.cookies.map((c) => ({
            key: c.key,
            domain: c.domain,
            path: c.path,
        }))));
    ...
    try {
        token =
            new URL(resp.url || ECARD_SERVICE_URL).searchParams.get("synjones-auth") || "";
    }
    catch {
        token = "";
    }
...
export const fetchCardInfo = async (username, password) => {
    const { sessionFetch, finalUrl, token } = await loginEcard(username, password);
    // 访问登录后的目标页面以确保凭证落盘
    await sessionFetch(finalUrl || ECARD_SERVICE_URL, { method: "GET" });
    const url = "https://ecard.csu.edu.cn/berserker-app/ykt/tsm/queryCard?scene=recharge&synAccessSource=pc";
    const headers = {
...
export const fetchCardTurnover = async (username, password, params) => {
    const { sessionFetch, finalUrl, token } = await loginEcard(username, password);
    await sessionFetch(finalUrl || ECARD_SERVICE_URL, { method: "GET" });
    const baseUrl = "https://ecard.csu.edu.cn/berserker-search/search/personal/turnover";
    const qs = new URLSearchParams();
    if (params.timeFrom)
        qs.set("timeFrom", params.timeFrom);
    if (params.timeTo)
...
const simplifyCardResponse = (raw) => {
...
const simplifyTurnoverResponse = (raw) => {
    if (!raw || typeof raw !== "object")
        return null;
    const data = raw.data ?? {};
