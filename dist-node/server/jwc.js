...
export const rank = async (user) => {
    ...
    if (cookiesBeforeRequest) {
        debug("rank cookies before request:", JSON.stringify(cookiesBeforeRequest.cookies.map((c) => ({
            key: c.key,
            domain: c.domain,
            path: c.path,
        }))));
    ...
    for (const term of terms) {
        ...
        debug("rank term status", term, termResp.status, "url", termResp.url);
...
export const classes = async (user, term, week) => {
    debug("classes start", { id: user.id, term, week });
    const form = new URLSearchParams();
    form.set("zc", week === "0" ? "" : week);
    form.set("xnxq01id", term);
    form.set("sfFD", "1");
    const resp = await authenticatedRequest(user.id, user.pwd, "POST", JWC_CLASS_URL, form);
    ...
    $("table#kbtable")
        ...
        $(row)
            ...
            const visibleBlocks = $(cell).find("div.kbcontent1");
            const blockCandidates = visibleBlocks.length > 0
                ? visibleBlocks
                : $(cell).find("div.kbcontent");
            blockCandidates.each((_, block) => {
                ...
                const className = (metaBlock.length > 0
                    ...
                    .text()
                    .trim();
                const fonts = (metaBlock.length > 0
                    ? metaBlock
                    : $block).find("font");
                const getByTitle = (keyword) => fonts
                    .filter((_, f) => ($(f).attr("title") ||
                    "").includes(keyword))
                    .first()
                    .text();
                const teacher = getByTitle("老师") ||
                    fonts.eq(0).text();
                const weeks = getByTitle("周次") ||
                    fonts.eq(1).text();
                const place = getByTitle("教室") ||
                    fonts.eq(2).text();
                if (className ||
                    teacher ||
                    weeks ||
    ...
    debug("classes parsed matrix length", classesMatrix.length, "startWeekDay", startWeekDay);
    return { classes: classesMatrix, startWeekDay };
};
export const levelExam = async (user) => {
    debug("levelExam start", { id: user.id });
    const resp = await authenticatedRequest(user.id, user.pwd, "GET", JWC_LEVEL_EXAM_URL, undefined);
...
export const studentPlan = async (user) => {
...
const _parseFilenameFromDisposition = (disposition) => {
    if (!disposition)
        return "";
    ...
    if (asciiName && /[^\x00-\x7f]/.test(asciiName)) {
        try {
            ...
            const decoded = new TextDecoder("gbk").decode(Buffer.from(asciiName, "binary"));
            if (decoded)
                return decoded;
        }
        catch {
...
export const studentInfo = async (user) => {
    ...
    if (!table.length) {
        debug("studentInfo page missing expected table", html.substring(0, 200));
    ...
    const findRow = (matcher) => rows.find((cells) => matcher(cells)) ?? [];
    const findRowIndex = (matcher) => rows.findIndex((cells) => matcher(cells));
    const joinedRow = (cells) => normalize(cells.join(" ").replace(/\s+/g, " "));
    const condensedRow = (cells) => normalize(cells.join("")).replace(/\s+/g, "");
    const firstNonEmpty = (values) => values.find((v) => v !== undefined && v !== "") ?? "";
    const baseInfoRow = findRow((cells) => cells.some((c) => c.includes("院系：")));
    ...
    const certificateRow = findRow((cells) => cells[0]?.includes("毕(结)业证书号"));
    const disciplineRow = findRow((cells) => cells.some((c) => c.includes("学科门类")));
    const printDateRow = findRow((cells) => cells.some((c) => c.includes("打印日期")));
    const info = {
        ...
        arrivalStation: addressRow[addressRow.length - 1] ??
        ...
        familyMembers: [],
        changes: [],
        disciplineCategory: disciplineRow
            .find((c) => c.includes("学科门类"))
            ?.replace(/^学科门类：?/, "") ?? "",
        graduationCertificate: certificateRow[1] ?? "",
        bachelorCertificate: certificateRow[certificateRow.length - 1] ?? "",
        printDate: printDateRow.find((c) => c.includes("打印日期")) ?? "",
    };
    const resumeHeaderIdx = findRowIndex((cells) => cells.join("").includes("学习及工作简历"));
    const familyHeaderIdx = findRowIndex((cells) => cells.join("").includes("家庭成员情况"));
    const changeHeaderIdx = findRowIndex((cells) => cells.join("").includes("学籍变动情况"));
    const flowHeaderIdx = findRowIndex((cells) => cells.join("").includes("大类分流"));
    const sectionRows = (startIdx, endIdx) => {
        if (startIdx < 0)
            return [];
        return rows.slice(startIdx + 2, endIdx > -1 ? endIdx : undefined);
    };
    const findSection = (startMatcher, endMatcher) => {
        const startIdx = findRowIndex((cells) => condensedRow(cells).includes(startMatcher));
        const endIdx = rows.findIndex((cells, idx) => idx > startIdx &&
            condensedRow(cells).includes(endMatcher));
        return sectionRows(startIdx, endIdx);
    };
    const cleanResumeRows = findSection("学习及工作简历", "家庭成员情况");
...
export const minorInfo = async (user) => {
    ...
    const planCache = {};
    const planTasks = [];
    const normalizePlanHref = (href) => {
        if (!href)
            return "";
        if (href.startsWith("javascript:")) {
            const match = /openWindow\(['"]([^'"]+)/.exec(href);
            if (match?.[1])
                return new URL(match[1], JWC_BASE_HOST).toString();
            return "";
        }
        return new URL(href, JWC_BASE_HOST).toString();
    };
    const fetchPlan = async (href) => {
        const normalized = normalizePlanHref(href.trim());
        if (!normalized)
            return [];
        // 避免重复请求同一培养方案
        if (planCache[normalized])
            return planCache[normalized];
        const planResp = await sessionFetch(normalized, {
            method: "GET",
        });
        debug("minorInfo plan status", planResp.status, "url", planResp.url);
    ...
    $reg(".Nsb_r_list tr").each((i, el) => {
        ...
        planTasks.push(fetchPlan(planLink.trim()).then((plan) => {
            last.Plan = plan;
        }));
    });
    // 辅修选课缴费
    ...
    debug("minorInfo parsed", "registrations", registrations.length, "payments", payments.length);
