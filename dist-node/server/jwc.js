...
export const summaryMarkdown = async (user, term = "") => {
    ...
    if (grades.length === 0) {
        ...
        md.push("| --- | --- | --- | --- | --- | --- |");
        grades.forEach((g) => md.push(`| ${g.GottenTerm} | ${g.ClassName} | ${g.FinalGrade} | ${g.Credit} | ${g.ClassNature} | ${g.ClassAttribute} |`));
    }
    md.push("");
    // 排名
    md.push("## 排名");
    if (ranks.length === 0) {
        ...
        md.push("| --- | --- | --- | --- |");
        ranks.forEach((r) => md.push(`| ${r.Term} | ${r.TotalScore} | ${r.ClassRank} | ${r.AverScore} |`));
    }
    md.push("");
    // 等级考试
    md.push("## 等级考试");
    if (levelExams.length === 0) {
        ...
        md.push("| 考试 | 笔试分 | 机试分 | 总分 | 笔试等级 | 机试等级 | 总等级 | 考试日期 |");
        md.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
        levelExams.forEach((e) => md.push(`| ${e.Course} | ${e.WrittenScore} | ${e.ComputerScore} | ${e.TotalScore} | ${e.WrittenLevel} | ${e.ComputerLevel} | ${e.TotalLevel} | ${e.ExamDate} |`));
    }
    md.push("");
    // 辅修报名与缴费
    md.push("## 辅修报名");
    if (minor.registrations.length === 0) {
        ...
        minor.registrations.forEach((reg, idx) => {
            md.push(`### 辅修报名 ${idx + 1}: ${reg.Major} (${reg.Department})`);
            md.push(`- 类型: ${reg.Type}  状态: ${reg.Status}`);
            if (reg.Plan.length === 0) {
                md.push("- 培养方案: 暂无数据");
            }
            else {
                md.push("| 序号 | 开课学期 | 课程编号 | 课程名称 | 学分 | 学时 | 考核方式 | 课程属性 | 是否考试 |");
                md.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- |");
                reg.Plan.forEach((p) => md.push(`| ${p.Index} | ${p.Term} | ${p.CourseId} | ${p.CourseName} | ${p.Credit} | ${p.Hours} | ${p.ExamType} | ${p.CourseAttr} | ${p.IsExam} |`));
            }
            md.push("");
        });
    }
    ...
    if (minor.payments.length === 0) {
        ...
        md.push("| 序号 | 课程编号 | 课程名称 | 开课单位 | 上课班级 | 上课地点 | 上课时间 | 教师 | 学分 | 学时 | 金额 | 是否缴费 |");
        md.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
        minor.payments.forEach((p) => md.push(`| ${p.Index} | ${p.CourseId} | ${p.CourseName} | ${p.Department} | ${p.Class} | ${p.Place} | ${p.Time} | ${p.Teacher} | ${p.Credit} | ${p.Hours} | ${p.Fee} | ${p.Paid} |`));
    }
    md.push("");
    // 执行计划 / 培养方案
    md.push("## 执行计划（培养方案）");
    if (plans.length === 0) {
        ...
        md.push("| 序号 | 开课学期 | 课程编号 | 课程名称 | 学分 | 总学时 | 考核方式 | 课程属性 | 是否考试 | 微调原因 |");
        md.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
        plans.forEach((p) => md.push(`| ${p.Index} | ${p.Term} | ${p.CourseId} | ${p.CourseName} | ${p.Credit} | ${p.Hours} | ${p.ExamType} | ${p.CourseAttr} | ${p.IsExam} | ${p.AdjustReason} |`));
    }
...
export const studentInfo = async (user) => {
    ...
    debug("studentInfo preflight status", preflight.status, "url", preflight.url);
    ...
    const contentType = resp.headers.get("content-type") || "application/msexcel";
    const buffer = await resp.arrayBuffer();
    // 为避免中文编码乱码，强制使用英文文件名
    const filename = `${user.id}-student-info.xls`;
    debug("studentInfo file ready", "size", buffer.byteLength, "filename", filename);
    return {
